...
| `--shared-coverage R` | 0.8 | 多场景时公共前置的门槛：一条采集要被这个比例的场景读到才留在公共层（两个场景 → 两个都读；十个 → 八个），否则下沉成各场景的「本场景采集」。分流判据与采集期判根因的采集不受此限 |
...
| 指标 | 健康值 | 不达标意味着 | 回哪一步 |
| --- | --- | --- | --- |
| 前置检查 命令数 / 步骤数 | 命令数 < 步骤数 × 3 | 一步塞了太多命令，回显读不完 | 拆分采集步骤 |
