...
**关于"无关的点和边"**：三类来源已经分别挡住了——跨诊断单元的靠 `--unit`，
...
4. **公共前置的规模**：`plan` 的「本场景采集」列——公共前置只放分流前必须跑的命令
   （场景覆盖率 ≥ 80%，或决定分流，或采集期判根因），其余下沉到读它的场景。
   公共前置条数居高不下，多半是场景边界画错了；门槛用 `--shared-coverage` 调。
