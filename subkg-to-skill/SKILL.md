...
**聚合边界（参考值）：一份 skill 装 5–7 个场景、30+ 根因、3–7 条公共前置、约 500 行。**
`plan` 会逐项对照着报，但**只报不拦**——超出不是错误，是交付时要说明、或回来再编排一轮的信号；
数据本身撑不到 30 个根因时，如实做小的那份，不要为凑数硬并。
...
1. **每个场景的步骤数**：明显偏厚（一个场景就占掉全篇一半）多半还能拆；只有 1 步且修复命令为 0
   的场景考虑并掉或不做。单条阅读路径超过 25 步 `lint` 会告警。
2. **修复命令数为 0 的场景**：生成出来只能定位不能处置，要么先不做，要么交付时说清楚。
3. **合并提示**：根因被完全包含 → 并；同一根因在多个场景各排一遍 → 说明场景边界画错了。
4. **公共前置的规模**：塌到 0–1 条或居高不下都说明边界画错了，回阶段二（门槛默认 80%，
   `--shared-coverage` 可调；单故障 skill 里同一条门槛按排查步骤算，只有一个步骤读、又不决定分流的采集还给那个步骤）。
...
| **命令里写死某台设备的取值** | 抽取时把案例里的 `slot 3`、`vlan 100` 一并带进 `command_templates` | 命令旁标「注意：命令含示例取值（…），按现场实际替换」，`lint` 同步告警；**不替换、不改名** |
...
- **参数名沿用来源写法**，只规整分隔符（`{interface-type}` → `<interface-type>`），不翻译、不换词；
  同一参数全篇同名，接口名用全称。命令里写死的取值（`slot 3`）**保留原样并留着那行注意**——
  自己改成 `<slot-id>` 是凭空造了一个来源里没有的参数，`verify` 会按无来源处理。
...
- 不要手工把案例里的 IP、设备名、`slot 3` 一类取值改成看起来通用的值或自造占位符——那是编造；
  要么剔除该条目，要么留着并保留注意行。
